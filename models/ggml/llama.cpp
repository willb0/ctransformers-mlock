...
static void llama_model_quantize_internal(
  ...
  llama_file_saver file_saver(fname_out.c_str(),
                              model_loader->file_loader.get(), params->ftype);
    ...
    if (!quantize) {
      ...
      if (tensor.name == "output.weight") {
        ...
        if (ftype == LLAMA_FTYPE_MOSTLY_Q3_K_M ||
          ...
          new_type = GGML_TYPE_Q6_K;
        // else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_S && i_feed_forward_w2 <
        // n_feed_forward_w2/8) new_type = GGML_TYPE_Q6_K;
        ++i_feed_forward_w2;
      ...
      if (new_type == GGML_TYPE_Q2_K || new_type == GGML_TYPE_Q3_K ||
          new_type == GGML_TYPE_Q4_K || new_type == GGML_TYPE_Q5_K ||
          new_type == GGML_TYPE_Q6_K) {
        int nx = tensor.ne.at(0);
        int ny = tensor.ne.at(1);
        if (nx % QK_K != 0 || ny % QK_K != 0) {
          fprintf(stderr,
                  "\n\nTensor sizes %d x %d are not divisible by %d, required "
                  "for k-quants.\n",
                  nx, ny, QK_K);
      ...
      if (convert_incompatible_tensor) {
        if (tensor.name == "output.weight") {
          new_type = GGML_TYPE_F16;  // fall back to F16 instead of just
                                     // failing.
          fprintf(stderr, "F16 will be used for this tensor instead.\n");
        } else if (tensor.name == "tok_embeddings.weight") {
          new_type =
              GGML_TYPE_Q4_0;  // fall back to Q4_0 instead of just failing.
...
int llama_apply_lora_from_file_internal(const struct llama_model &model,
  ...
  if (path_base_model) {
    fprintf(stderr, "%s: loading base model from '%s'\n", __func__,
            path_base_model);
    model_loader.reset(
        new llama_model_loader(path_base_model, /*use_mmap*/ true));
    ...
    if (model_loader->use_mmap) {
      model_loader->mapping.reset(new llama_mmap(
          &model_loader->file_loader->file, /* prefetch */ 0, ggml_is_numa()));
    }
  }
...
#ifdef GGML_USE_CUBLAS
      if (dest_t->backend == GGML_BACKEND_GPU ||
          dest_t->backend == GGML_BACKEND_GPU_SPLIT) {
        if (dest_t->type != GGML_TYPE_F16) {
          throw std::runtime_error(
              format("%s: error: the simultaneous use of LoRAs and GPU "
                     "acceleration is only supported for f16 models",
                     __func__));
        }
        offload_func = ggml_cuda_assign_buffers;
...
int llama_apply_lora_from_file(struct llama_context *ctx, const char *path_lora,
  ...
  try {
    return llama_apply_lora_from_file_internal(ctx->model, path_lora,
                                               path_base_model, n_threads);
  } catch (const std::exception &err) {
...
int llama_model_apply_lora_from_file(const struct llama_model *model,
                                     const char *path_lora,
                                     const char *path_base_model,
                                     int n_threads) {
  try {
    return llama_apply_lora_from_file_internal(*model, path_lora,
                                               path_base_model, n_threads);
  } catch (const std::exception &err) {
    fprintf(stderr, "%s: failed to apply lora adapter: %s\n", __func__,
            err.what());
...
/** copy state data into either a buffer or file depending on the passed in
 * context
 *
 * file context:
 ...
 *
 */
void llama_copy_state_data_internal(struct llama_context *ctx,
                                    llama_data_context *data_ctx) {
  // copy rng
  {
  ...
  // copy logits
  {
    ...
    if (padding_size > 0) {
      std::vector<uint8_t> padding(padding_size,
                                   0);  // Create a buffer filled with zeros
...
static bool llama_load_session_file_internal(struct llama_context *ctx,
...
bool llama_load_session_file(struct llama_context *ctx,
                             ...
                             size_t *n_token_count_out) {
  try {
    return llama_load_session_file_internal(
...
void llama_print_timings(struct llama_context *ctx) {
  ...
  fprintf(stderr,
          ...
          __func__, timings.t_sample_ms, timings.n_sample,
  ...
  fprintf(stderr,
          ...
          "%8.2f tokens per second)\n",
          __func__, timings.t_p_eval_ms, timings.n_p_eval,
  ...
  fprintf(stderr,
          ...
          "%8.2f tokens per second)\n",
          __func__, timings.t_eval_ms, timings.n_eval,
